package com.zdh.alphathink.imh.CustomWidgets;
...
public class ClearWriteEditText extends android.support.v7.widget.AppCompatEditText implements View.OnFocusChangeListener,TextWatcher {
    ...
    /**
     *置图标的显示与隐藏，setCompoundDrawables为EditText绘制
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (getCompoundDrawables()[2] != null){
            if (event.getAction() == MotionEvent.ACTION_UP){
                ...
                mClearDrawable.getIntrinsicWidth())&&(event.getX() < (getWidth() - getPaddingRight()));
                if (touchable){
                    this.setText("");
                }
            }
    ...
    /**
     * 当ClearEditText焦点发生变化时，判断里面字符串长度设置清除图标的设置与隐藏
     */
